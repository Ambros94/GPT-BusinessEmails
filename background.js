...
function createContextMenu() {
    ...
    chrome.contextMenus.create({
        id: 'GPT-Prompter',
        title: 'GPT-Prompter ', // documentUrlPatterns: ["<all_urls>"], // this will make it work on local chrome pages, but the content script will not work
    ...
    // if the context menu clicked is the on-the-fly prompt
        ...
        chrome.tabs.query({active: true, currentWindow: true}, function (tabs) {
            chrome.tabs.sendMessage(tabs[0].id, {
                ...
                text: selectionText,
                body_data: {"model": "text-davinci-003", "temperature": 0, "max_tokens": 2048}
            });
        });
    }
});
