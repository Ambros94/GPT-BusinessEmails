...
class popUpClass extends HTMLElement {
    ...
    ontheflypopup(selectionText) {
        this.shadowRoot.innerHTML += flypopup(this.ids, {
            text: selectionText,
        ...
        id_textarea.addEventListener('keydown', (e) => {
            e.stopPropagation();
        });
        ...
        if (stream) {
            ...
            chrome.storage.local.get('history', function (items) {
                if (typeof items.history !== 'undefined') {
                    items.history.push([message.body_data, complete_completion, 0]);// add the result to the history
                    chrome.storage.local.set({'history': items.history});
                } else {
                    items.history = [[message.body_data, complete_completion, 0]]; // initialize the history array
                    chrome.storage.local.set({'history': items.history});
                }
            });
        }
